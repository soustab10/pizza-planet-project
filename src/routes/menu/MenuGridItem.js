...
const MenuGridItem = ({ singleProduct, handleAddProduct, handleRemoveProduct }) => {
  ...
  return (
    <article className="menu-grid-item flex-container flex-column txt-white">
      <Link onClick={ResetLocation} to={`/menu/${singleProduct.pizza_id}`} className="menu-item-link">
        <img src={singleProduct.pizza_image_url} alt={`${singleProduct.pizza_name}`} />
      </Link>
      ...
      <div className="price">
        {singleProduct.sale === true ?
          <section className="sale-pricing">
            <p className="price-num-before"><span>$</span>{singleProduct.price}</p>
